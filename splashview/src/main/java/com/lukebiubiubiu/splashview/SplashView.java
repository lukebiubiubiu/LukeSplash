...
public class SplashView extends View {
    ...
    private Splash[][] mParticles = new Splash[ROW_NUM][COLUMN_NUM];
    private Splash[][] mMinParticles = new Splash[ROW_NUM][COLUMN_NUM];
    ...
    private void initView(AttributeSet attrs) {
        ...
        TypedArray typeArray = getContext().obtainStyledAttributes(attrs, R.styleable.SplashView);
        mHostText = null == typeArray.getString(R.styleable.SplashView_pv_host_text) ? "" : typeArray.getString(R.styleable.SplashView_pv_host_text);
        mParticleText = null == typeArray.getString(R.styleable.SplashView_pv_particle_text) ? "" : typeArray.getString(R.styleable.SplashView_pv_particle_text);
        mParticleTextSize = (int) typeArray.getDimension(R.styleable.SplashView_pv_particle_text_size, DEFAULT_MIN_TEXT_SIZE);
        int hostTextSize = (int) typeArray.getDimension(R.styleable.SplashView_pv_host_text_size, DEFAULT_MIN_TEXT_SIZE);
        mBgColor = typeArray.getColor(R.styleable.SplashView_pv_background_color, 0xFF0867AB);
        mParticleColor = typeArray.getColor(R.styleable.SplashView_pv_text_color, 0xFFCEF4FD);
        mTextAnimTime = typeArray.getInt(R.styleable.SplashView_pv_text_anim_time, DEFAULT_TEXT_ANIM_TIME);
        mSpreadAnimTime = typeArray.getInt(R.styleable.SplashView_pv_text_anim_time, DEFAULT_SPREAD_ANIM_TIME);
        mHostTextAnimTime = typeArray.getInt(R.styleable.SplashView_pv_text_anim_time, DEFAULT_HOST_TEXT_ANIM_TIME);
        typeArray.recycle();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mMinParticles[i][j] = new Splash(mStartMinP.x + (mEndMinP.x - mStartMinP.x) / COLUMN_NUM * j, mStartMinP.y + (mEndMinP.y - mStartMinP.y) / ROW_NUM * i, dip2px(0.8f));
            }
        }
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mParticles[i][j] = new Splash(mStartMaxP.x + (mEndMaxP.x - mStartMaxP.x) / COLUMN_NUM * j, mStartMaxP.y + (mEndMaxP.y - mStartMaxP.y) / ROW_NUM * i, getTextWidth(mHostText + mParticleText, mParticleTextPaint) / (COLUMN_NUM * 1.8f));
            }
        }
            ...
            for (int j = 0; j < COLUMN_NUM; j++) {
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        mParticles[tempI][tempJ] = (Splash) animation.getAnimatedValue();
